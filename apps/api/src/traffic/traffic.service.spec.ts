...
describe('TrafficService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    trafficRepo = new Repository<TrafficRecord>(
      TrafficRecord,
      null as any as EntityManager,
    );
    cache = {
  ...
  describe('byCountry', () => {
    it('converts SUM string to number', async () => {
      jest
        .spyOn(trafficRepo, 'createQueryBuilder')
        .mockImplementation(
          () =>
            mockQueryBuilder as unknown as SelectQueryBuilder<TrafficRecord>,
        );
      mockQueryBuilder.getRawMany.mockResolvedValue([
        { name: 'UAE', vehicles: '18200' },
      ]);
