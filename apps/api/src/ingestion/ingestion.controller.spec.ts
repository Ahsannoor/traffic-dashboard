...
describe('IngestionService', () => {
    ...
    beforeEach(() => {
        ...
        ingestionService = new IngestionService(trafficRepo, cache as unknown as Cache);
    });
    ...
    it('merges new counts into the cached country aggregate when present', async () => {
        ...
        cache.get.mockImplementation(async (key: string) =>
            key === 'traffic:by-country::' ? [{ name: 'Oman', vehicles: 1000 }] : undefined,
        );

        await ingestionService.upsertBatch(records as any);
